...
exports.booking = async (req,res) => {
    try {
        ...
        if(user!=null){
            let coach=await model.coaches.findOne({coachId:req.params.coachId});
            if(coach!=null){
                if(
                    validator.slot(req.body.slot) &&
                    validator.appointmentDate(req.body.dateOfAppointment) &&
                    await validator.slotAvailability(user.userId,coach.coachId,req)
                ){
                    let newBooking={
...
exports.updateBooking = async (req,res) => {
    try {
        let booking=await model.bookings.findOne({bookingId: req.params.bookingId});
        if(booking!=null){
            if(
                validator.slot(req.body.slot) &&
                validator.appointmentDate(req.body.dateOfAppointment) &&
                await validator.slotAvailability(booking.userId,booking.coachId,req)
            ){
...
exports.deleteBooking = async (req,res) => {
    try {
        let del=await model.bookings.deleteOne({bookingId: req.params.bookingId});
